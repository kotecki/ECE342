* TLV6001 - Rev. A 
*** DEK: 10/19/2017 modified for NGspice compatibility 
***   -  changed | to || for 'or' function - line 355
***   -  changed switches from VSWITCH to vs
***   -  changed model for R_NOISELESS
***   -  changed nodes with +/- to p/n (NGspice doesn't like +/-)
***   -  need to add .func statements for IF and LIMIT to hspc file
***
***
...
.model BB_SW sw(Ron=50 Roff=1e9 Vt=350e-3 Vh=-350e-3)
.model ESD_SW sw(Ron=50 Roff=1e9 Vt=300e-3 Vh=-200e-3)
.model OL_SW sw(Ron=1e-3 Roff=1e9 Vt=850e-3 Vh=-50e-3)
.model OR_SW sw(Ron=10e-3 Roff=1e9 Vt=5e-4 Vh=5e-4)
*
***.model R_NOISELESS RES(T_ABS=-273.15)
.model R_NOISELESS R(TNOM=0)
******************************************************
V_OS N032 N038 595.218e-6
...
XVCM_CLAMP N036 MID N039 MID N042 N043 VCCS_EXT_LIM
...
XCL_AMP N003 N034 VIMON MID N013 N026 CLAMP_AMP_LO
...
C9 MID N014 1e-15
XCLAW_AMP VCC_CLP VEE_CLP VOUT_S MID N011 N024 CLAMP_AMP_LO
...
C11 MID N012 1e-15
XCL_SRC N014 N027 CL_CLAMP MID VCCS_LIM_4
XCLAW_SRC N012 N025 CLAW_CLAMP MID VCCS_LIM_3
...
XGR_AMP N008 N037 N021 MID N022 N030 CLAMP_AMP_HI
XGR_SRC N023 N031 CLAMP MID VCCS_LIM_GR
C17 MID N023 1e-15
C18 N031 MID 1e-15
V_GRn N037 MID -160
V_GRp N008 MID 160
...
V_ORn N015 VCLP -1.8
E1 MID 0 N056 0 1
S8 N018 CLAMP CLAMP N018 OR_SW off
S9 CLAMP N017 N017 CLAMP OR_SW off
Xe_n N038 ESDp VNSE
Xi_nn ESDn MID FEMT
Xi_np N038 MID FEMT
XVCCS_LIMIT_1 N040 N044 MID N041 VCCS_LIM_1
XVCCS_LIMIT_2 N041 MID MID CLAMP VCCS_LIM_2
...
XOL_SENSE MID N060 N059 N062 OL_SENSE
...
S1 N049 N048 SW_OL MID OL_SW off
XVCCS_LIM_ZO N051 MID MID N052 VCCS_LIM_ZO
R57 N052 MID R_NOISELESS 1
.ends TLV6001
*
.subckt CLAMP_AMP_HI VC+ VC- VIN COM VO+ VO-
.param G=10
GVop COM Vo+ Value = {IF(V(VIN,COM)>V(VC+,COM),((V(VIN,COM)-V(VC+,COM))*G),0)}
GVon COM Vo- Value = {IF(V(VIN,COM)<V(VC-,COM),((V(VC-,COM)-V(VIN,COM))*G),0)}
.ends CLAMP_AMP_HI
*
.subckt FEMT 1 2
.param FLWF=1e-3
...
GSWp 1 2 Value = {IF((V(3,1)>10e-3 || V(4,1)>10e-3),1,0)}
.ends OL_SENSE
*
